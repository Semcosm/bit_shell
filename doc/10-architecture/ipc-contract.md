...
```json
{
  ...
  "changed": ["bar.*", "dock.*", "shell.auto_reconnect_niri"],
  "hot_applied": ["bar.*", "dock.*", "shell.auto_reconnect_niri"],
  "restart_required": [],
  "config_loaded": true
}
```
...
- 当前 `bar.*` 走 `StateStore -> IPC event -> bit_bar` 运行时更新通路
...
- `launchpad.*` 当前尚无运行时消费者，因此只会出现在 `restart_required`
