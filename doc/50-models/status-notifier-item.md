...
- `IconPixmap` 与 `AttentionIconPixmap` 当前已沿 `tray` topic / snapshot 透传到前端；前端会优先用 theme icon，其次用 pixmap，再退回文本。dbusmenu 树同步与多 host 管理仍后置到后续阶段；`shell.tray_watcher_name` 已支持通过 app 层重建 tray service 热切换。
- 前端图标选择顺序当前是：可用 theme icon 优先，其次匹配 slot 的 pixmap，最后才退回文本 fallback。
- `bit_bar` 当前已引入 tray menu bridge：它只拥有 popup 生命周期、anchor 与单实例打开策略，不拥有 dbusmenu 树本身。
